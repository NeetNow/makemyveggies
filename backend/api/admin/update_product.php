...
try {
    ...
    $existsStmt = $pdo->prepare('SELECT product_id FROM products WHERE product_id = ?');
    ...
    $delImgs->execute([$productId]);

    $imgSql = 'INSERT INTO product_images (product_id, image_url, is_primary) VALUES (:product_id, :image_url, :is_primary)';
    $imgStmt = $pdo->prepare($imgSql);

    $imgStmt->bindValue(':product_id', $productId, PDO::PARAM_INT);
    $imgStmt->bindValue(':image_url', $primaryImageUrl);
    $imgStmt->bindValue(':is_primary', 1, PDO::PARAM_INT);
    $imgStmt->execute();

    if (!empty($secondaryImageUrls)) {
        foreach ($secondaryImageUrls as $url) {
            $imgStmt->bindValue(':product_id', $productId, PDO::PARAM_INT);
            $imgStmt->bindValue(':image_url', $url);
            $imgStmt->bindValue(':is_primary', 0, PDO::PARAM_INT);
            $imgStmt->execute();
        }
    }
