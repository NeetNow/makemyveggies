...
const Categories = () => {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/backend/api/admin/get_categories.php`, {
        method: 'GET',
        credentials: 'include'
      });
  ...
  const pageRows = categories.slice(startIndex, startIndex + ROWS_PER_PAGE);
  ...
  const submitAdd = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/backend/api/admin/add_category.php`, {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const submitEdit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/backend/api/admin/update_category.php`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json'
  ...
  const submitDelete = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/backend/api/admin/delete_category.php`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json'
          ...
          {categories.length > ROWS_PER_PAGE && (
            <div className="d-flex align-items-center justify-content-between mt-3">
              <div className="text-muted small">
                Showing {startIndex + 1}-{Math.min(startIndex + ROWS_PER_PAGE, categories.length)} of {categories.length}
              </div>
