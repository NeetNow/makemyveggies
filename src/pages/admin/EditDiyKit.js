...
const EditDiyKit = () => {
  ...
  useEffect(() => {
    if (!form.categoryId && diyChildCategories.length > 0) {
      setForm((p) => ({ ...p, categoryId: String(diyChildCategories[0].id) }));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [diyChildCategories.length]);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/admin/products/diy-kits/${encodeURIComponent(String(id))}`);
    } catch (e2) {
  ...
  return (
    <div className="admin-form-page pb-2">
      <div className="d-flex align-items-center justify-content-between mb-3 admin-page-header">
        <div>
          <h4 className="mb-0">Edit DIY Kit</h4>
          <div className="text-muted small">Update product details</div>
        </div>
        <button type="button" className="btn btn-secondary btn-sm" onClick={() => navigate(`/admin/products/diy-kits/${encodeURIComponent(String(id || ''))}`)}>
          Back
        </button>
      </div>
        ...
        <form onSubmit={onSubmit} className="admin-form" style={{ maxWidth: 1100 }}>
          <div className="row g-3 admin-form__row">
            ...
            <div className="col-12 d-flex gap-2">
              ...
              <button type="button" className="btn btn-outline-secondary" disabled={saving} onClick={() => navigate(`/admin/products/diy-kits/${encodeURIComponent(String(id || ''))}`)}>
                Cancel
              </button>
            </div>
          </div>
