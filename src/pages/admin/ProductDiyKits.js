...
const ProductDiyKits = () => {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        const match = list.find((c) => (c?.name || '').trim().toLowerCase() === DIY_CATEGORY);
  ...
  const diyResolvedCategoryId = useMemo(() => {
    if (diyCategoryId) return diyCategoryId;
    const match = categories.find((c) => (c?.name || '').trim().toLowerCase() === DIY_CATEGORY);
    return match?.id || null;
  }, [categories, diyCategoryId, DIY_CATEGORY]);

  const diyChildCategories = useMemo(() => {
    if (!diyResolvedCategoryId) return [];
    return categories
      .filter((c) => Number(c?.parentId) === Number(diyResolvedCategoryId))
      .sort((a, b) => String(a?.name || '').localeCompare(String(b?.name || '')));
  }, [categories, diyResolvedCategoryId]);
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (childCategoryId) {
        qs.set('category', String(childCategoryId));
      } else {
        qs.set('category', DIY_CATEGORY);
      }
      ...
      if (!response.ok || data?.status !== 'success') {
        throw new Error(data?.message || 'Failed to load DIY kits');
      }
      ...
      setError(e?.message || 'Failed to load DIY kits');
      setProducts([]);
      setTotalPages(1);
      setTotalCount(0);
    } finally {
  ...
  const openViewPage = (p) => {
    if (!p?.id) return;
    navigate(`/admin/products/diy-kits/${encodeURIComponent(String(p.id))}`);
  };

  const onEdit = (p) => {
    if (!p?.id) return;
    navigate(`/admin/products/diy-kits/${encodeURIComponent(String(p.id))}/edit`);
  };
  ...
  return (
    <div className="container-fluid">
        ...
        <div className="card-body py-2">
          <div className="row g-2">
            ...
            <div className="col-12 col-sm-6 col-lg-3">
              ...
              >
                <option value="">All DIY Kits</option>
      ...
      {!loading && !error && (
        <>
          {products.length === 0 ? (
            <div className="text-muted">No DIY kits found.</div>
          ) : (
              ...
              {products.map((p) => {
                ...
                return (
                  <div key={p.id} className="col-12 col-sm-6 col-lg-4">
                    ...
                    >
                      <div className="ratio ratio-4x3 bg-light" style={{ borderTopLeftRadius: 16, borderTopRightRadius: 16, overflow: 'hidden' }}>
                        <img
                          src={img}
                          alt={p.title}
          ...
          {totalPages > 1 && (
            <div className="d-flex align-items-center justify-content-between mt-3">
              ...
              <nav aria-label="DIY kits pagination">
