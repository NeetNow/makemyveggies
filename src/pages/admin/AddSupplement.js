...
const AddSupplement = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok || data?.status !== 'success') {
        throw new Error(data?.message || 'Failed to add supplement');
      }

      toast.success('Supplement added successfully');
      navigate('/admin/products/supplements');
    } catch (e2) {
      toast.error(e2?.message || 'Failed to add supplement');
    } finally {
  ...
  return (
    <div className="admin-form-page pb-2">
      <div className="d-flex align-items-center justify-content-between mb-3 admin-page-header">
        <div>
          <h4 className="mb-0">Add Supplement</h4>
          <div className="text-muted small">Create a new supplement product</div>
        </div>
          ...
          <div className="row g-3 admin-form__row">
            ...
            <div className="col-12 d-flex gap-2">
              ...
              <button
                ...
                disabled={submitting}
                onClick={() => navigate('/admin/products/supplements')}
              >
                Cancel
              </button>
            </div>
